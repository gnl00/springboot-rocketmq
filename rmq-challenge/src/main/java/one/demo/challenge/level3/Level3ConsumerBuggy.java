...
@RocketMQMessageListener(
        topic = "order-payment",
        tag = "payment-buggy",
        consumerGroup = "order-consumer-buggy",
        endpoints = "localhost:8080"
)
public class Level3ConsumerBuggy implements RocketMQListener {
    ...
    public ConsumeResult consume(MessageView messageView) {
        try {
            ...
            // Bug 3: 没有做幂等性保护，直接更新订单状态
            // 虽然更新订单状态本身是幂等的，但如果结合其他操作（如发货、发优惠券），就会有问题
            orderService.updateOrderToPaid(orderId);

            log.info("支付处理成功 - OrderId: {}, UserId: {}, Amount: {}",
                    orderId, userId, amount);

            // Bug 4: 即使处理成功，也没有记录这条消息已经处理过
            // 如果 ACK 之前进程崩溃，消息会重新投递，导致重复消费
