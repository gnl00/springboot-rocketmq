...
@RocketMQMessageListener(
        ...
        tag = "*",
        consumerGroup = "notification-consumer-fixed",
        endpoints = "localhost:8080",
        consumptionThreadCount = 30
        // Bug 1: 没有配置消费并发度，默认可能只有 1 个线程
        // Bug 2: 没有配置批量消费
)
public class Level4ConsumerFixed implements RocketMQListener {
        ...
        try {
            ...
            asyncProcessExecutor.execute(() -> {
                try {
                    ...
                    log.error("processSlowOperation error, e=", e);
                }
            });
